...
func ProcessStatusEQ(v ProcessStatus) predicate.Order {
	return predicate.Order(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldProcessStatus), v))
	})
}

// ProcessStatusNEQ applies the NEQ predicate on the "process_status" field.
func ProcessStatusNEQ(v ProcessStatus) predicate.Order {
	return predicate.Order(func(s *sql.Selector) {
		s.Where(sql.NEQ(s.C(FieldProcessStatus), v))
	})
}

// ProcessStatusIn applies the In predicate on the "process_status" field.
func ProcessStatusIn(vs ...ProcessStatus) predicate.Order {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return predicate.Order(func(s *sql.Selector) {
		...
		s.Where(sql.In(s.C(FieldProcessStatus), v...))
	})
}

// ProcessStatusNotIn applies the NotIn predicate on the "process_status" field.
func ProcessStatusNotIn(vs ...ProcessStatus) predicate.Order {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return predicate.Order(func(s *sql.Selector) {
		...
		s.Where(sql.NotIn(s.C(FieldProcessStatus), v...))
	})
}
...
func HasDiscounts() predicate.Order {
	return predicate.Order(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(DiscountsTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, DiscountsTable, DiscountsPrimaryKey...),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}
...
func HasDiscountsWith(preds ...predicate.Discount) predicate.Order {
	return predicate.Order(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
