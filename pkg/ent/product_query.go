...
// QueryShoppingCartOwners chains the current query on the "shopping_cart_owners" edge.
func (pq *ProductQuery) QueryShoppingCartOwners() *UserQuery {
...
// WithShoppingCartOwners tells the query-builder to eager-load the nodes that are connected to
// the "shopping_cart_owners" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithShoppingCartOwners(opts ...func(*UserQuery)) *ProductQuery {
	query := &UserQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) sqlAll(ctx context.Context) ([]*Product, error) {
	...
	if query := pq.withOrders; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(product.OrdersPrimaryKey[1], fks...))
			},
	...
	if query := pq.withCategories; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(product.CategoriesPrimaryKey[0], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(uuid.UUID), new(uuid.UUID)}
			},
	...
	if query := pq.withShoppingCartOwners; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[uuid.UUID]*Product, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.ShoppingCartOwners = []*User{}
		}
		var (
			edgeids []uuid.UUID
			edges   = make(map[uuid.UUID][]*Product)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(product.ShoppingCartOwnersPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(uuid.UUID), new(uuid.UUID)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, pq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "shopping_cart_owners": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "shopping_cart_owners" node returned %v`, n.ID)
			}
			for i := range nodes {
