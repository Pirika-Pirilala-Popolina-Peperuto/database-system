...
func (oq *OrderQuery) QueryDiscounts() *DiscountQuery {
	query := &DiscountQuery{config: oq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withDiscounts: oq.withDiscounts.Clone(),
...
func (oq *OrderQuery) WithDiscounts(opts ...func(*DiscountQuery)) *OrderQuery {
	query := &DiscountQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
	oq.withDiscounts = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context) ([]*Order, error) {
	...
	if query := oq.withProducts; query != nil {
		...
		ids := make(map[uuid.UUID]*Order, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(order.ProductsPrimaryKey[0], fks...))
			},
	...
	if query := oq.withDiscounts; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[uuid.UUID]*Order, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Discounts = []*Discount{}
		}
		var (
			edgeids []uuid.UUID
			edges   = make(map[uuid.UUID][]*Order)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(order.DiscountsPrimaryKey[0], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(uuid.UUID), new(uuid.UUID)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, oq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "discounts": %w`, err)
		}
		query.Where(discount.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "discounts" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Discounts = append(nodes[i].Edges.Discounts, n)
			}
		}
	}
