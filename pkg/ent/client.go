...
// UpdateOneID returns an update builder for the given id.
func (c *DiscountClient) UpdateOneID(id uuid.UUID) *DiscountUpdateOne {
...
// DeleteOneID returns a delete builder for the given id.
func (c *DiscountClient) DeleteOneID(id uuid.UUID) *DiscountDeleteOne {
...
func (c *DiscountClient) Get(ctx context.Context, id uuid.UUID) (*Discount, error) {
	return c.Query().Where(discount.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *DiscountClient) GetX(ctx context.Context, id uuid.UUID) *Discount {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryOrders queries the orders edge of a Discount.
func (c *DiscountClient) QueryOrders(d *Discount) *OrderQuery {
...
func (c *OrderClient) QueryProducts(o *Order) *ProductQuery {
	...
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(order.Table, order.FieldID, id),
			sqlgraph.To(product.Table, product.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, order.ProductsTable, order.ProductsPrimaryKey...),
		)
		fromV = sqlgraph.Neighbors(o.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryDiscounts queries the discounts edge of a Order.
func (c *OrderClient) QueryDiscounts(o *Order) *DiscountQuery {
	...
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, order.DiscountsTable, order.DiscountsPrimaryKey...),
		)
		fromV = sqlgraph.Neighbors(o.driver.Dialect(), step)
		return fromV, nil
	}
...
func (c *ProductClient) QueryOrders(pr *Product) *OrderQuery {
	...
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.To(order.Table, order.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, product.OrdersTable, product.OrdersPrimaryKey...),
		)
...
// QueryShoppingCartOwners queries the shopping_cart_owners edge of a Product.
func (c *ProductClient) QueryShoppingCartOwners(pr *Product) *UserQuery {
	query := &UserQuery{config: c.config}
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		id := pr.ID
		step := sqlgraph.NewStep(
...
// QueryShoppingCartProducts queries the shopping_cart_products edge of a User.
func (c *UserClient) QueryShoppingCartProducts(u *User) *ProductQuery {
	query := &ProductQuery{config: c.config}
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
			sqlgraph.To(product.Table, product.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, user.ShoppingCartProductsTable, user.ShoppingCartProductsPrimaryKey...),
		)
		fromV = sqlgraph.Neighbors(u.driver.Dialect(), step)
		return fromV, nil
	}
