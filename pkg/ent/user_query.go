...
type UserQuery struct {
	...
	withShoppingCartProducts *ProductQuery
...
// QueryShoppingCartProducts chains the current query on the "shopping_cart_products" edge.
func (uq *UserQuery) QueryShoppingCartProducts() *ProductQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withShoppingCartProducts: uq.withShoppingCartProducts.Clone(),
...
// WithShoppingCartProducts tells the query-builder to eager-load the nodes that are connected to
// the "shopping_cart_products" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithShoppingCartProducts(opts ...func(*ProductQuery)) *UserQuery {
	query := &ProductQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withShoppingCartProducts = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		loadedTypes = [2]bool{
			uq.withOrders != nil,
			uq.withShoppingCartProducts != nil,
		}
	)
	...
	if query := uq.withShoppingCartProducts; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[uuid.UUID]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.ShoppingCartProducts = []*Product{}
		}
		var (
			edgeids []uuid.UUID
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.ShoppingCartProductsPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "shopping_cart_products": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "shopping_cart_products" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ShoppingCartProducts = append(nodes[i].Edges.ShoppingCartProducts, n)
			}
		}
	}
