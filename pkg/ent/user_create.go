...
func (uc *UserCreate) AddShoppingCartProductIDs(ids ...uuid.UUID) *UserCreate {
	uc.mutation.AddShoppingCartProductIDs(ids...)
	return uc
}

// AddShoppingCartProducts adds the "shopping_cart_products" edges to the Product entity.
func (uc *UserCreate) AddShoppingCartProducts(p ...*Product) *UserCreate {
	ids := make([]uuid.UUID, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uc.AddShoppingCartProductIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.ShoppingCartProductsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
